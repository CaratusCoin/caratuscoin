...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCaratusAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCaratusAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCaratusAmount = strAnonymizeCaratusAmount.remove(strAnonymizeCaratusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCaratusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCaratusAmount*COIN) nMaxToAnonymize = nAnonymizeCaratusAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCaratusAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCaratusAmount));
        strAnonymizeCaratusAmount = strAnonymizeCaratusAmount.remove(strAnonymizeCaratusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCaratusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
