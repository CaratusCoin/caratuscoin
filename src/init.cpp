...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Caratus.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Caratusd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeCaratusamount=<n> " + _("Keep N Caratus anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Caratus is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Caratus is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Caratus version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Caratus server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Caratus") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Caratus to complete") << "\n";
    ...
    nAnonymizeCaratusAmount = GetArg("-anonymizeCaratusamount", 0);
    if(nAnonymizeCaratusAmount > 999999) nAnonymizeCaratusAmount = 999999;
    if(nAnonymizeCaratusAmount < 2) nAnonymizeCaratusAmount = 2;
    ...
    LogPrintf("Anonymize Caratus Amount %d\n", nAnonymizeCaratusAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1CARAT+1000 == (.1CARAT+100)*10
       10CARAT+10000 == (1CARAT+1000)*10
    */
